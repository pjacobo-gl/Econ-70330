...
```{r TBOND, echo=FALSE, warning=FALSE}
#--- Actuals ---#
...
TBOND_actual <- TBOND_actual %>%
  mutate(
    YEAR = year(observation_date),
    QUARTER = quarter(observation_date)
  ) %>%
  select(YEAR, QUARTER, DGS10) %>%
  rename(actual = DGS10)


#--- Forecasts ---#
# Import data
TBOND <- read_excel("/Users/jacobogomezlandero/Documents/Notre Dame PhD/Coursework/Field courses/Expectations/Replications/Econ-70330/Data/SPFmicrodata.xlsx", sheet = "TBOND")
...
TBOND_cols <- grep("^TBOND", names(TBOND), value = TRUE)

# Winsorize extreme forecasts: more than 5×IQR from median in a given quarter & horizon
TBOND_long <- TBOND %>%
  pivot_longer(cols = all_of(TBOND_cols),
               names_to = "horizon",
               values_to = "forecast_level") %>%
  mutate(forecast_level = suppressWarnings(as.numeric(forecast_level)))
TBOND_long_trimmed <- TBOND_long %>%
  group_by(YEAR, QUARTER, horizon) %>%
  mutate(
    med = median(forecast_level, na.rm = TRUE),
    iqr = IQR(forecast_level, na.rm = TRUE),
  ...
  ungroup() %>%
  select(-med, -iqr, -lower, -upper)
TBOND_trimmed <- TBOND_long_trimmed %>%
  pivot_wider(names_from = horizon, values_from = forecast_level)

# Keep only forecasters with ≥ 10 valid observations
TBOND_trimmed <- TBOND_trimmed %>%
  group_by(ID) %>%
  mutate(n_obs = n()) %>%
  ungroup() %>%
  filter(n_obs >= 10) %>%
...
rm(TBOND_long_trimmed, TBOND_long, TBOND)


#--- CB Regressions ---#
TBOND_indiv <- TBOND_trimmed %>%
  arrange(ID, YEAR, QUARTER) %>%
  group_by(ID) %>%
  mutate(
    ...
    prev_forecast_same_target = lag(TBOND6, 1),
  ...
  left_join(TBOND_actual, by = c("YEAR", "QUARTER")) %>%
...
cg_consensus_reg <- lm(forecast_error ~ forecast_revision, data = TBOND_consensus)
...
# Create a unique time cluster ID
TBOND_indiv <- TBOND_indiv %>%
  mutate(time_id = paste0(YEAR, "Q", QUARTER))

# Run pooled OLS
pool_reg <- lm(forecast_error ~ forecast_revision, data = TBOND_indiv)

# Two-way clustered SEs (by ID and by time)
cl_vcov <- cluster.vcov(pool_reg, TBOND_indiv[, c("ID", "time_id")])
pool_results <- coeftest(pool_reg, vcov = cl_vcov)
pool_results

# median beta1 across forecasters
library(broom)
beta_by_id <- TBOND_indiv %>%
  group_by(ID) %>%
  group_modify(~ {
...
TBILL_actual <- TBILL_actual %>%
  mutate(
    YEAR = year(observation_date),
    QUARTER = quarter(observation_date)
  ) %>%
  select(YEAR, QUARTER, DGS3MO) %>%
  rename(actual = DGS3MO)


#--- Forecasts ---#
# Import data
TBILL <- read_excel("/Users/jacobogomezlandero/Documents/Notre Dame PhD/Coursework/Field courses/Expectations/Replications/Econ-70330/Data/SPFmicrodata.xlsx", sheet = "TBILL")
...
# Identify NOUTPUT columns
TBILL_cols <- grep("^TBILL", names(TBILL), value = TRUE)

# Winsorize extreme forecasts: more than 5×IQR from median in a given quarter & horizon
TBILL_long <- TBILL %>%
  pivot_longer(cols = all_of(TBILL_cols),
               names_to = "horizon",
               values_to = "forecast_level") %>%
  mutate(forecast_level = suppressWarnings(as.numeric(forecast_level)))
TBILL_long_trimmed <- TBILL_long %>%
  group_by(YEAR, QUARTER, horizon) %>%
  mutate(
    med = median(forecast_level, na.rm = TRUE),
    iqr = IQR(forecast_level, na.rm = TRUE),
  ...
  ungroup() %>%
  select(-med, -iqr, -lower, -upper)
TBILL_trimmed <- TBILL_long_trimmed %>%
  pivot_wider(names_from = horizon, values_from = forecast_level)

# Keep only forecasters with ≥ 10 valid observations
TBILL_trimmed <- TBILL_trimmed %>%
  group_by(ID) %>%
  mutate(n_obs = n()) %>%
  ungroup() %>%
  filter(n_obs >= 10) %>%
...
rm(TBILL_long_trimmed, TBILL_long, TBILL)


#--- CB Regressions ---#
TBILL_indiv <- TBILL_trimmed %>%
  arrange(ID, YEAR, QUARTER) %>%
  group_by(ID) %>%
  mutate(
    # Individual i's forecast of year-ahead nominal yield growth (if you're using ratios)
    ...
    # Previous quarter's forecast for the same target period
    prev_forecast_same_target = lag(TBILL6, 1),
  ...
  ungroup() %>%
  # Join the actual yield data
  left_join(TBILL_actual, by = c("YEAR", "QUARTER")) %>%
  # Compute the forecast error *after* the join
  mutate(
    forecast_error = actual - forecast
  )
...
cg_consensus_reg <- lm(forecast_error ~ forecast_revision, data = TBILL_consensus)
...
# Create a unique time cluster ID
TBILL_indiv <- TBILL_indiv %>%
  mutate(time_id = paste0(YEAR, "Q", QUARTER))

# Run pooled OLS
pool_reg <- lm(forecast_error ~ forecast_revision, data = TBILL_indiv)

# Two-way clustered SEs (by ID and by time)
cl_vcov <- cluster.vcov(pool_reg, TBILL_indiv[, c("ID", "time_id")])
pool_results <- coeftest(pool_reg, vcov = cl_vcov)
pool_results

# median beta1 across forecasters
library(broom)
beta_by_id <- TBILL_indiv %>%
  group_by(ID) %>%
  group_modify(~ {
    df <- tidyr::drop_na(.x, forecast_error, forecast_revision)
    if (nrow(df) >= 2) {
...
```{r BOND, echo=FALSE, warning=FALSE}
#--- Actuals ---#

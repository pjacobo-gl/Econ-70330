...
sce_fitted <- sce %>%
  ...
  ungroup() %>%
  dplyr::mutate(
    mean_infl = map_dbl(fit, "mean", .default = NA_real_),
    var_infl  = map_dbl(fit, "var",  .default = NA_real_),
    p1        = map_dbl(fit, "p1",   .default = NA_real_),
    p99       = map_dbl(fit, "p99",  .default = NA_real_),
    dist_type = map_chr(fit, "type", .default = "none")
  ) %>%
  dplyr::select(-fit)
...
sce_trim1 <- sce_fitted %>% filter(Q8v2part2 >= p1, Q8v2part2 <= p99)
n1 <- nrow(sce_trim1)
